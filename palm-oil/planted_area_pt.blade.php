@extends('layouts.vertical', ['title' => 'Planted Area'])
...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
  ...
  GNU nano 7.2                                resources/views/palm-oil/planted_area_pt.blade.php                                         
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Planted Area Bulanan </h4>
</h5>
            </div>

            <div class="card-body" style="overflow-x: auto;">

    <h4>Detail Planted Area: {{ $perusahaan }}</h4>
            ...
            <tr>
                ...
                @foreach (range(1, 12) as $i)
                    @php
                        ...
                        $val = $row ? $row->total_planted_inti : 0;
                        $total += $val;
                    @endphp
            ...
            <tr>
                ...
                @foreach (range(1, 12) as $i)
                    @php
                        $bulan = str_pad($i, 2, '0', STR_PAD_LEFT);
                        $row = $items->firstWhere('bulan', $bulan);
                        $val = $row ? $row->total_planted_plasma : 0;
                        $total += $val;
                    @endphp
                    <td>{{ number_format($val, 0, ',', '.') }}</td>
                @endforeach
