    ...
    <div class="col-12">
        <div class="card">
           <div class="card-header">
                <h5 class="card-title mb-0">New Planting </h5>  
            </div>
...
@foreach ($data as $key => $rows)
    ...
    @endphp
    <tr>
        <td><a href="{{ route('palm-oil.new_planting_pt', ['id' => $id, 'perusahaan' => $perusahaan]) }}">
{{ $perusahaan }}</a></td>
        <td>{{ $area }}</td>
        @foreach ($tahunRange as $tahun)
            @php
                $row = $groupedByYear->get($tahun);
    $inti = isset($row->new_planting_inti) ? floatval(str_replace(',', '.', $row->new_planting_inti)) : 0;
     $plasma = isset($row->new_planting_plasma) ? floatval(str_replace(',', '.', $row->new_planting_plasma)) : 0;
