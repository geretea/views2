...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <h4 class="fs-18 fw-semibold m-0">Area</h4>
    </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Lahan Konservasi</h5>
            </div>
    ...
    <tbody>
        ...
        <tr>
            ...
            @foreach(array_reverse($years) as $year)
                ...
                @endphp
                <td>{{ number_format($record->total_dalam_hgu, 0, ',', '.') }}</td>
                <td>{{ number_format($record->total_luar_hgu, 0, ',', ',') }}</td>
            @endforeach
        </tr>
        @endforeach
					</tbody>
            ...
            @foreach(array_reverse($years) as $year)
				...
				<td>
                    @if($total)
{{ $totalPerTahun[$year]->total_dalam_hgu ?? 0 }}
					<td>
{{ $totalPerTahun[$year]->total_luar_hgu ?? 0 }}
                    @else
                        -
                    @endif
            @endforeach
