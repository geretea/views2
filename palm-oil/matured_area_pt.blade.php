@extends('layouts.vertical', ['title' => 'Matured Area'])
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Matured Area Bulanan </h4>
</h5>
            </div>

            <div class="card-body" style="overflow-x: auto;">

    <h4>Detail Matured Area: {{ $perusahaan }}</h4>
    ...
    <table class="table table-bordered">
        ...
        @foreach($data as $tahun => $items)
            {{-- Inti --}}
            <tr>
                                                resources/views/palm-oil/matured_area_pt.blade.php                                         
                ...
                @foreach (range(1, 12) as $i)
                    @php
                        ...
                        $val = $row ? $row->mature_area_inti : 0;
                        $total += $val;
                    @endphp
            ...
            <tr>
                ...
                @foreach (range(1, 12) as $i)
                    @php
                        $bulan = str_pad($i, 2, '0', STR_PAD_LEFT);
                        $row = $items->firstWhere('bulan', $bulan);
                        $val = $row ? $row->mature_area_plasma : 0;
                        $total += $val;
                    @endphp
                    <td>{{ number_format($val, 0, ',', '.') }}</td>
                @endforeach
