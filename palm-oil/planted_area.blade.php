    ...
    <div class="col-12">
        <div class="card">
           <div class="card-header">
                <h5 class="card-title mb-0">Planted Area </h5>  
            </div>
...
@foreach ($data as $key => $rows)
    ...
    @endphp
    <tr>
        <td><a href="{{ route('palm-oil.planted_area_pt', ['id' => $id, 'perusahaan' => $perusahaan]) }}">
{{ $perusahaan }}</a></td>
        <td>{{ $area }}</td>
        @foreach ($tahunRange as $tahun)
            @php
                $row = $groupedByYear->get($tahun);
                $inti = $row->total_planted_inti ?? 0;
                $plasma = $row->total_planted_plasma ?? 0;
		    $total = $inti + $plasma;
                $totals['inti'] += $inti;
