@extends('layouts.vertical', ['title' => 'Sales FFB'])
...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <h4 class="fs-18 fw-semibold m-0">Sales FFB</h4>
    </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Data Sales FFB per PT</h5>
            </div>
...
<div class="row">
   <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Sales Volume FFB {{ request('perusahaan', 'all') != 'all' ? ' - ' . request('perusahaan') : '' }}
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
</h5>
                </div>
                <div class="card-body">
                    <canvas id="barChartFFB_Volume"></canvas> 
                </div>
            </div>  
        </div>
   <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Sales Revenue FFB {{ request('perusahaan', 'all') != 'all' ? ' - ' . request('perusahaan') : '' }}
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
</h5>
                </div>
                <div class="card-body">
                    <canvas id="barChartFFB_Revenue"></canvas> 
                </div>
            </div>  
        </div>
        <div class="row">
    <div class="col-lg-6">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Total Sales Volume FFB  {{ request('perusahaan', 'all') != 'all' ? ' - ' . request('perusahaan') : '' }}
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
</h5>       </div>
     ...
     <div class="col-lg-6">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Total Sales Revenue FFB  {{ request('perusahaan', 'all') != 'all' ? ' - ' . request('perusahaan') : '' }}
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
</h5>       </div>
...
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
    ...
    <div class="card-body">
                    <canvas id="lineChartFFB_Volume"></canvas> 
                </div>
            </div>  
        </div>
<div class="col-md-6">
...
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
    ...
    <div class="card-body">
                    <canvas id="lineChartFFB_Revenue"></canvas> 
                </div>
            </div>  
        </div>
</div>
...
<script>
    ...
    new Chart(document.getElementById('barChartFFB_Volume'), {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
                label: 'Sales Volume FFB',
  ...
  new Chart(document.getElementById('barChartFFB_Revenue'), {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
                label: 'Sales Revenue FFB',
    ...
    new Chart(document.getElementById('lineChartFFB_Volume'), {
        type: 'line',
        data: {
            labels: labels,
            datasets: [
  ...
  new Chart(document.getElementById('lineChartFFB_Revenue'), {
        type: 'line',
        data: {
            labels: labels,
            datasets: [
