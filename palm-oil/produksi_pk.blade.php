@extends('layouts.vertical', ['title' => 'Produksi Palm Kernel'])
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Produksi Palm Kernel (PK)</h5>
            </div>
 ...
 <tbody>
    ...
    <tr class="fw-bold bg-light">
        <td colspan="2" class="text-center">Total Produksi</td>
        @for ($year = 2024; $year >= 2022; $year--)
            <td>
                {{ number_format(
                    collect($groupedData)->flatMap(fn($areas) => $areas)
                        ->sum(fn($data) => optional($data->firstWhere('tahun', $year))->total_aktual ?? 0), 
                    0
                ) }}
            </td>
        @endfor
