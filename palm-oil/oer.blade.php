...
<div class="row">
    <div class="col-12">
        <div class="card">
          <canvas id="oerLineChart" height="300"></canvas>
       </div>
    </div>
</div>
...
<div class="row">
 ...
 <a href="{{ url('palm-oil/' . $id . '/oer_bulanan') }}" class="btn btn-outline-primary">
        Cek Data OER Bulanan
    </a>
            </div>
    ...
    <tr>
        ...
        @foreach ($tahunList as $th)
            @php
                ...
                $oer = ($row && $row->total_ffb > 0)
                    ? round(($row->total_aktual / $row->total_ffb) * 100, 2)
                    : '-';
            @endphp
            <td class="text-end">{{ is_numeric($oer) ? number_format($oer, 2, ',', '.') . ' %' : '-' }}</td>
        @endforeach
    </tr>
@endforeach
<tr class="fw-bold bg-light">
    ...
    @foreach ($tahunList as $th)
        <td class="text-end">
            {{ is_numeric($oerTotalPerTahun[$th] ?? null) 
                ? number_format($oerTotalPerTahun[$th], 2, ',', '.') . ' %' 
                : '-' }}
        </td>
    @endforeach
</tr>
