@extends('layouts.vertical', ['title' => 'Aplikasi LA'])
@section('css')
    @vite([
...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <h4 class="fs-18 fw-semibold m-0">Aplikasi LA</h4>
    </div>
    ...
    <div class="col-12">
        <div class="card">
           <div class="card-header">
                <h5 class="card-title mb-0">>Monitoring Land Application (LA) Tahun {{ $tahunAktif }})</h5>  
            </div>
   <div class="card-body">
    ...
    <tbody>
        @forelse($filtered as $row)
              <tr>
                <td>{{ $row->pt }}</td>
                <td class="text-end">{{ number_format($row->plan_volume, 0,',','.') }}</td>
                <td class="text-end">{{ number_format($row->plan_hektar,0,',','.') }}</td>
                <td class="text-end">{{ number_format($row->real_volume, 0,',','.') }}</td>
                <td class="text-end">{{ number_format($row->real_hektar, 0,',','.') }}</td>
<td class="text-end">
    {{ $row->persen !== null ? number_format($row->persen, 2, ',', '.') . ' %' : '-' }}
</td>                <td class="text-end">{{ number_format($row->plan_dosis, 2,',','.') }}</td>
                <td class="text-end">{{ number_format($row->real_dosis, 2,',','.') }}</td>
            </tr>
        @empty
            <tr>
