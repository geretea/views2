...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <h4 class="fs-18 fw-semibold m-0">Produksi CPO per PKS</h4>
    </div>
...
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">Total Produksi CPO  {{ request('pks', 'all') != 'all' ? ' - ' . request('pks') : '' }}
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
</h5>       </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ number_format($cpoTotal[$i], 0, ',', '.') }}</td>
                </tr>
                @endforeach
                <tr class="table-secondary">
                    ...
                    <th>{{ number_format(array_sum($cpoTotal), 0, ',', '.') }}</th>
                </tr>
            </tbody>
        </table>
        </div>
...
<div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Produki CPO {{ request('pks', 'all') != 'all' ? ' - ' . request('pks') : '' }}
{{ request('mode', 'bulanan') == 'tahunan' ? ' (' . request('tahun', $latestYear) . '–' . request('tahun_akhir', $latestYear) . ')' : ' (' . request('tahun', $latestYear) . ')' }}
                        </h5> 
                        </div>                   
    <div class="card-body">
                    <canvas id="lineChartCPO"></canvas> 
                </div>
            </div>  
        </div>
...
<script>
    ...
    const cpoData = @json($cpoTotal);

    new Chart(document.getElementById('produksiCPOChart'), {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
    ...
    new Chart(document.getElementById('lineChartCPO'), {
        type: 'line',
        data: {
            labels: labels,
            datasets: [
