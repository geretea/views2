...
<body>
    <div class="container">
        <h1>Review Dokumen Corsec</h1>
        @if(auth()->user()->role === 'checker' && $document->status === 'review')
    <a href="{{ route('corporate.corsec.review', $document->id) }}" class="btn btn-warning">Review Ulang</a>
@endif
         <form action="{{ route('corporate.corsec.review.submit', $document->id ?? '') }}" method="POST">
            @csrf
