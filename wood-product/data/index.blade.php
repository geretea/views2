@extends('layouts.vertical', ['title' => 'Dokumen WP'])
...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <h4 class="fs-18 fw-semibold m-0">Wood Product</h4>
    </div>

    <div class="text-end">
        <ol class="breadcrumb m-0 py-0">
            <li class="breadcrumb-item"><a href="javascript: void(0);">Data</a></li>
            <li class="breadcrumb-item active">Dokumen WP</li>
        </ol>
    </div>
</div>
...
@endif
    ...
    @endif
    <a href="{{ route('wood-product.data.upload', $id ?? 0) }}" class="btn btn-primary mb-3">Submit Dokumen</a>
    <div class="card-body">
            ...
            @forelse($dokumen as $item)
                ...
                <td>
                    @if(auth()->user()->role === 'checker' && $item->status === 'draft')
                        <a href="{{ route('wood-product.data.review', $item->id) }}" class="action-btn btn-review">Review</a>
                    @endif
                    @if(auth()->user()->role === 'checker' && $item->status === 'rejected')
    <a href="{{ route('wood-product.data.review', $item->id) }}" class="btn btn-warning">Review Ulang</a>
@endif
                    @if(auth()->user()->role === 'approver' && $item->status === 'review')
                        <a href="{{ route('wood-product.data.approve', $item->id) }}" class="action-btn btn-approve">Approve</a>
                    @endif
                </td>
                </tr>
            @empty
